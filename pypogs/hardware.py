...
class Camera:
    ...
    def __init__(self,
        ...
        self._logger.debug("Creating instance. Constructor input: Model:" +
                           str(model) + " ID:" + str(identity) + " Name:" +
                           str(name) + " AutoInit:" + str(auto_init))
        self._model = None
        self._identity = None
    ...
    def _ptgrey_release(self):
        ...
        if self._ptgrey_system is not None:
            self._log_debug("Has PtGrey system. Is in use? " +
                            str(self._ptgrey_system.IsInUse()))
    ...
    def model(self, model):
        ...
        model = str(model)
        assert (
            model.lower() in self._supported_models
    ...
    def identity(self, identity):
        ...
        if self.model.lower() == "ptgrey":
            ...
            if not self._ptgrey_system:
                self._ptgrey_system = PySpin.System.GetInstance(
                )  # Get singleton
            self._ptgrey_camlist = self._ptgrey_system.GetCameras()
            self._log_debug("Got cam list, size:" +
                            str(self._ptgrey_camlist.GetSize()))
            ...
            if valid:
                self._log_debug("Already init: " +
                                str(self._ptgrey_camera.IsInitialized()))
            if not valid:
                ...
                self._log_debug(
                    "Seems valid. Setting identity and cleaning up")
                del self._ptgrey_camera
                self._ptgrey_camera = None
            ...
            raise RuntimeError("An unknown (forbidden) model is defined: " +
                               str(self.model))
    ...
    def is_init(self):
        ...
        if not self.model:
            return False
        if self.model.lower() == "ptgrey":
            init = (self._ptgrey_camera is not None
                    and self._ptgrey_camera.IsInitialized())
            return init
        else:
            self._log_warning("Forbidden model string defined.")
            raise RuntimeError("An unknown (forbidden) model is defined: " +
    ...
    def initialize(self):
        ...
        assert not None in (
            self.model,
            self.identity,
        ), "Must define model and identity before initialising"
        if self.model.lower() == "ptgrey":
            ...
            self._ptgrey_camera = self._ptgrey_camlist.GetBySerial(
                self.identity)
            ...
            class PtGreyEventHandler(PySpin.ImageEvent):
                ...
                def OnImageEvent(self, img_ptr):
                    ...
                    try:
                        img = img_ptr.GetData().reshape(
                            (img_ptr.GetHeight(), img_ptr.GetWidth()))
                        ...
                        self.parent._log_warning("Failed to unpack image",
                                                 exc_info=True)
                        self.parent._image_data = None
                    finally:
                        img_ptr.Release()
                    self.parent._got_image_event.set()
                    self.parent._log_debug("Time: " +
                                           str(self.parent._image_timestamp) +
                                           " Size:" +
                                           str(self.parent._image_data.shape) +
                                           " Type:" +
                                           str(self.parent._image_data.dtype))
                    for func in self.parent._call_on_image:
                        try:
                            self.parent._log_debug("Calling back to: " +
                                                   str(func))
                            func(self.parent._image_data,
                                 self.parent._image_timestamp)
                        except:
                            self.parent._log_warning("Failed image callback",
                                                     exc_info=True)
    ...
    def deinitialize(self):
        ...
        if self._ptgrey_camera:
            ...
            try:
                ...
                self._ptgrey_camera = None
                self._log_debug(
                    "Deinitialised PtGrey camera object and deleted")
            except:
    ...
    def flip_x(self):
        ...
        if self.model.lower() == "ptgrey":
            self._log_debug(
                "Using PtGrey camera. Will flip the received image array ourselves: "
                + str(self._flipX))
            return self._flipX
        else:
            self._log_warning("Forbidden model string defined.")
    ...
    def rotate_90(self, k):
        ...
        if self.model.lower() == "ptgrey":
            self._log_debug(
                "Using PtGrey camera. Will rotate the received image array ourselves."
            )
            self._rot90 = k
    ...
    def frame_rate_auto(self, auto):
        ...
        if self.model.lower() == "ptgrey":
            ...
            if not PySpin.IsAvailable(node) or not PySpin.IsWritable(node):
                self._log_debug(
                    "Node not available or not writable. Available:" +
                    str(PySpin.IsAvailable(node)) + " Writable:" +
                    str(PySpin.IsWritable(node)))
    ...
    def frame_rate_limit(self):
        ...
        if self.model.lower() == "ptgrey":
            ...
            if not PySpin.IsAvailable(node1) or not PySpin.IsAvailable(node2):
                self._log_debug("One node not available. Node1:" +
                                str(PySpin.IsAvailable(node1)) + " Node2:" +
                                str(PySpin.IsAvailable(node2)))
                raise RuntimeError("Unable to read from camera")
            else:
                ...
                try:
                    ...
                    if "OutOfRangeException" in e.message:
                        raise AssertionError(
                            "The commanded value is outside the allowed range. See frame_rate_limit"
                        )
                    else:
        ...
        if self.model.lower() == "ptgrey":
            ...
            if not PySpin.IsAvailable(node) or not PySpin.IsWritable(node):
                ...
                try:
                    ...
                    if "OutOfRangeException" in e.message:
                        raise AssertionError(
                            "The commanded value is outside the allowed range. See gain_limit"
                        )
                    else:
                        raise  # Rethrows error
        else:
    ...
    def exposure_time(self, exposure_ms):
        ...
        if self.model.lower() == "ptgrey":
            ...
            if not PySpin.IsAvailable(node) or not PySpin.IsWritable(node):
                ...
                self._log_debug("Setting exposure time to (us): " +
                                str(exposure_ms * 1000))
                try:
        ...
        if self.model.lower() == "ptgrey":
            ...
            try:
                ...
                if val_horiz != val_vert:
                    self._log_warning(
                        "Horzontal and vertical binning is not equal.")
                return val_horiz
    ...
    def binning(self, binning):
        ...
        if self.is_running:
            self._log_debug(
                "Camera is running, stop it and restart immediately after.")
            self.stop()
        initial_size = self.size_readout
        initial_bin = self.binning
        self._log_debug("Initial sensor readout area and binning: " +
                        str(initial_size) + " ," + str(initial_bin))
        if self.model.lower() == "ptgrey":
            self._log_debug("Using PySpin")
            import PySpin
        ...
        self._log_debug("New binning and new size to set: " + str(new_bin) +
                        " ," + str(new_size))
        try:
            self.size_readout = new_size
            self._log_debug("Set new size to: " + str(self.size_readout))
        except:
            self._log_warning("Failed to scale readout after binning change",
                              exc_info=True)
        if was_running:
            try:
                self.start()
    ...
    def size_readout(self, size):
        ...
        if self.model.lower() == "ptgrey":
            ...
            try:
                ...
                self._log_warning("Failure reading max size to center",
                                  exc_info=True)
            new_offset = None
            try:
                ...
                self._log_debug("Actual and max, w,h {} {} {} {}".format(
                    actual_w, actual_h, max_size[0], max_size[1]))
                new_offset = (
                    round((max_size[0] - actual_w) / 2),
            ...
            if new_offset is not None:
                try:
                    ...
                    self._log_warning("Failure centering readout",
                                      exc_info=True)
        else:
            self._log_warning("Forbidden model string defined.")
            raise RuntimeError("An unknown (forbidden) model is defined: " +
        ...
        if self.model.lower() == "ptgrey":
            ...
            try:
                ...
                if "already streaming" in e.message:
                    ...
                    raise RuntimeError(
                        "Failed to start camera acquisition") from e
        else:
            self._log_warning("Forbidden model string defined.")
            raise RuntimeError("An unknown (forbidden) model is defined: " +
                               str(self.model))
    ...
    def get_next_image(self, timeout=10):
        ...
        if not self.is_running:
            self._log_debug(
                "Camera was not running, start and grab the first image")
            self._got_image_event.clear()
            self.start()
...
class Mount:
    ...
    def __init__(self,
        ...
        # Start of constructor
        self._logger.debug("Mount called with model=" + str(model) +
    ...
    def __del__(self):
        ...
        try:
            self._logger.debug(
                "Destructor called, try stop moving and disconnecting")
        except:
            pass
        try:
            self.stop()
    ...
    def identity(self, identity):
        ...
        assert not self.is_init, "Can not change already intialised device"
        assert isinstance(identity,
                          (str, int)), "Identity must be a string or an int"
        assert self.model is not None, "Must define model first"
        if self.model == "celestron":
            if isinstance(identity, int):
                ...
                try:
                    ...
                    raise AssertionError("No serial port for index: " +
                                         str(identity))
            self._logger.debug(
                "Using celestron with string identity, try to open and check model"
            )
            try:
                with serial.Serial(
                        identity,
                    ...
                    assert len(r) == 2 and r[1] == ord(
                        "#"
                    ), "Did not get the expected response from the device"
            except serial.SerialException:
                self._logger.debug("Failed to open port", exc_info=True)
                raise AssertionError("Failed to open the serial port named: " +
                                     str(identity))
            self._identity = identity
        else:
    ...
    def alt_limit(self, altlim):
        if altlim is None:
            ...
            assert (isinstance(altlim, (tuple, list))
                    and len(altlim) == 2), "Must be 2-tuple"
    ...
    def azi_limit(self, azilim):
        ...
        assert isinstance(
            azilim, (tuple, list)) and len(azilim) == 2, "Must be 2-tuple"
        self._logger.debug("Got set azi limits with: " + str(azilim))
        self._azi_limit = (
            float(azilim[0]) if azilim[0] is not None else None,
    ...
    def initialize(self):
        ...
        if self.model == "celestron":
            ...
            try:
                self._cel_serial_port = serial.Serial(
                    self.identity,
                    9600,
                    parity=serial.PARITY_NONE,
                    stopbits=serial.STOPBITS_ONE,
                ...
                raise RuntimeError(
                    "Failed to connect to the mount during initialisation")
            ...
            self._logger.debug("Got response: " + str(r))
            assert len(r) == 2 and r[1] == ord(
                "#"), "Did not get the expected response during initialisation"
    ...
    def deinitialize(self):
        ...
        if self.model == "celestron":
            self._logger.debug(
                "Using celestron, closing and deleting serial port")
    ...
    def is_moving(self):
        ...
        if self.model == "celestron":
            ...
            self._logger.debug("Mount returned: " + str(ret[0]) +
                               ", is moving: " + str(moving))
            return moving
        else:
            self._logger.warning("Forbidden model string defined.")
            raise RuntimeError("An unknown (forbidden) model is defined: " +
    ...
    def move_to_alt_az(self, alt, azi, block=True, rate_control=True):
        ...
        """
        assert self.is_init, "Must be initialised"
        assert (self._alt_limit[0] is None
                or alt >= self._alt_limit[0]), "Altitude outside range!"
        assert (self._alt_limit[1] is None
                or alt <= self._alt_limit[1]), "Altitude outside range!"
        assert (self._azi_limit[0] is None
                or azi >= self._azi_limit[0]), "Azimuth outside range!"
        assert (self._azi_limit[1] is None
                or azi <= self._azi_limit[1]), "Azimuth outside range!"
        self._logger.debug("Got move command with: alt=" + str(alt) + " azi=" +
                           str(azi) + " block=" + str(block) +
        ...
        if self.model == "celestron":
            ...
            self._logger.debug("Will command: alt=" + str(alt) + " azi=" +
                               str(azi))
            if rate_control:  # Use own control thread
                ...
                def _loop_slew_to(alt, azi, success):
                    while not self._control_thread_stop:
                        ...
                        if eAlt < -self._max_speed[0]:
                            eAlt = -self._max_speed[0]
                        if eAlt > self._max_speed[0]:
                            eAlt = self._max_speed[0]
                        if eAzi < -self._max_speed[1]:
                            eAzi = -self._max_speed[1]
                        if eAzi > self._max_speed[1]:
                            eAzi = self._max_speed[1]

                        if abs(eAlt) < 0.001 and abs(eAzi) < 0.001:
                ...
                self._control_thread = Thread(target=_loop_slew_to,
                                              args=(alt, azi, success))
                ...
                def _move_to_alt_az(alt, azi, success):
                    ...
                    altRaw = (int(
                        self.degrees_to_0_360(alt - self._alt_zero) / 360 *
                        2**32)
                        & 0xFFFFFF00)
                    aziRaw = (int(self.degrees_to_0_360(azi) / 360 * 2**32)
                              & 0xFFFFFF00)
    ...
    def get_alt_az(self):
        ...
        if self.model == "celestron":
            ...
            t.start()
            t.join()
            alt = self.degrees_to_n180_180(
                float(ret[0]) / 2**32 * 360 + self._alt_zero)
            azi = self.degrees_to_n180_180(float(ret[1]) / 2**32 * 360)
            self._logger.debug("Mount returned: alt=" + str(ret[0]) + " azi=" +
                               str(ret[1]) + " => alt=" + str(alt) + " azi=" +
                               str(azi))
    ...
    def move_home(self, block=True, rate_control=True):
        ...
        Args:
            ...
            rate_control (bool, optional): If True (the default) the rate of the mount will be controlled until position
                is reached, if False the position command will be sent to the mount for excecution.
        """
        self.move_to_alt_az(*self.home_alt_az,
                            block=block,
    ...
    def set_rate_alt_az(self, alt, azi):
        ...
        """
        assert self.is_init, "Must be initialised"
        self._logger.debug("Got rate command. alt=" + str(alt) + " azi=" +
                           str(azi))
        if (abs(alt) > self._max_speed[0]) or (abs(azi) > self._max_speed[0]):
            raise ValueError("Above maximum speed!")
        if self.model == "celestron":
            self._logger.debug(
                "Using celestron, sending rate command to mount")
            success = [False]
            ...
            def _set_rate_alt_az(alt, azi, success):
                ...
                if rate >= 0:
                    rateLo = rate & 0xFF
                    rateHi = rate >> 8 & 0xFF
                    self._cel_send_bytes_command(
                        [ord("P"), 3, 17, 6, rateHi, rateLo, 0, 0])
                else:
                    rateLo = -rate & 0xFF
                    rateHi = -rate >> 8 & 0xFF
                    self._cel_send_bytes_command(
                        [ord("P"), 3, 17, 7, rateHi, rateLo, 0, 0])
                assert self._cel_check_ack(), "Mount did not acknowledge!"
                # Azimuth
                ...
                if rate >= 0:
                    ...
                    rateHi = rate >> 8 & 0xFF
                    self._cel_send_bytes_command(
                        [ord("P"), 3, 16, 6, rateHi, rateLo, 0, 0])
                else:
                    rateLo = -rate & 0xFF
                    rateHi = -rate >> 8 & 0xFF
                    self._cel_send_bytes_command(
                        [ord("P"), 3, 16, 7, rateHi, rateLo, 0, 0])
                assert self._cel_check_ack(), "Mount did not acknowledge!"
                success[0] = True
    ...
    def wait_for_move_to(self, timeout=120):
        ...
        raise TimeoutError("Waiting for mount move took more than " +
                           str(timeout) + "seconds.")
    ...
    def list_available_ports():
        ...
        return ([(x.device, x.description)
                 for x in port_list] if len(port_list) > 0 else [])
...
class Receiver:
    ...
    def __init__(
        ...
        # Start of constructor
        self._logger.debug("Receiver called with: model=" + str(model) +
                           " identity=" + str(identity) + " name=" +
                           str(name) + " auto_init=" + str(auto_init) +
    ...
    def identity(self, identity):
        ...
        if self.model.lower() == "ni_daq":
            self._logger.debug(
                "Using NI DAQ, checking vailidity by opening a task")
            ...
            try:
                ...
                try:
                    ...
                    self._logger.debug(
                        "Failed to delete task used to test identity",
                        exc_info=True)
        else:
            raise RuntimeError("An unknown (forbidden) model is defined: " +
                               str(self.model))
    ...
    def initialize(self):
        ...
        if self.model.lower() == "ni_daq":
            ...
            try:
                self._ni_task = (ni.Task(self.name)
                                 if self.name is not None else ni.Task())
            except ni.DaqError:
            ...
            try:
                ...
                self._logger.info("Successfully initialised" +
                                  (": " +
                                   self.name) if self.name is not None else "")
                self._is_init = True
            except ni.DaqError:
    ...
    def sample_rate(self, rate_hz):
        ...
        assert isinstance(
            rate_hz,
            (float, int)), "Sample rate must be a scalar (float or int)"
        assert self.is_init, "Must initialise first"
        ...
        if self.model.lower() == "ni_daq":
            ...
            assert 0 < rate_hz <= self._ni_task.timing.samp_clk_max_rate, (
                "Requested rate is not allowed, " + "maximum rate is: " +
                str(self._ni_task.timing.samp_clk_max_rate))
            try:
                self._ni_task.timing.cfg_samp_clk_timing(
                    rate=rate_hz,
                    sample_mode=ni.constants.AcquisitionType.CONTINUOUS)
                self._logger.debug("Sampling rate set to: " +
                                   str(self._ni_task.timing.samp_clk_rate))
            except:
    ...
    def measurement_range(self, meas_range):
        ...
        assert isinstance(meas_range, (
            int, float,
            tuple)), "Input must be scalar (int/float) or a 2-tuple of scalars"
        if isinstance(meas_range, tuple):
            assert (
                len(meas_range) == 2
            ), "Input must be scalar (int or float) or a 2-tuple of scalars"
            assert all(
                isinstance(x, (int, float)) for x in meas_range
            ), "Input must be scalar (int or float) or a 2-tuple of scalars"
        else:
        ...
        if self.model.lower() == "ni_daq":
            ...
            self._logger.debug("Using NI DAQ, trying to set")
            self._logger.debug(
                "NOTE: nidaqmx is broken, must manually check if range is allowed (-10, 10)..."
            )
            assert (min(meas_range) >= -10
                    and max(meas_range) <= 10), "Values must be <=10 and >=-10"
            ...
            try:
                ...
                self._logger.debug("Range set to: " +
                                   str(self.measurement_range))
            except ni.DaqError:
                self._logger.exception(
                    "Failed to set new values, this may cause strange behaviour. De-init and re-init."
                )
        else:
            raise RuntimeError("An unknown (forbidden) model is defined: " +
                               str(self.model))
    ...
    def start(self):
        ...
        if self.model.lower() == "ni_daq":
            ...
            cb_count = int(min(1000, max(
                1, self.sample_rate /
                10)))  # Typically 10Hz, min 1 and max 1000 per callback

            def _ni_buffering_callback(task_handle, event_type,
            ...
            try:
                ...
                self._logger.debug("Registered event every n=" +
                                   str(cb_count) + " samples")
            except ni.DaqError:
                self._logger.debug(
    ...
    def _get_update_from_hardware(self):
        ...
        if self.model.lower() == "ni_daq":
            ...
            try:
                ...
                self._logger.debug("Data of length " + str(len(data)) +
                                   " and class " + str(type(data)))
            except:
                data = None
                if self.is_running:
    ...
    def _update_stored_values(self, data):
        ...
        if None in (
            ...
            self._logger.debug("Previous smooth and instant powers are: " +
                               str(self._smooth_power) + " " +
                               str(self._instant_power))
            self._instant_power = data[-1]  # Last read data saved here
            k = len(data)
            a = 1 / self._smoothing_parameter
    ...
    def _create_data_file(self):
        ...
        header = ("TIME: " + timestamp.isoformat() + "; " + "NAME: " +
                  str(self.name) + "; " + "MODEL: " + str(self.model) + "; " +
                  "IDENTITY: " + str(self.identity) + "; " + "SAMPLE_RATE: " +
                  str(self.sample_rate) + "; " + "MEASUREMENT_RANGE: " +
                  str(self.measurement_range) + "; " + "FORMAT: " +
                  "STRUCT_PACK_FLOAT32" + ";\n")
