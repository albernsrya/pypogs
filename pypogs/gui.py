...
class GUI:
    ...
    def __init__(self, pypogs_system, gui_update_ms=500, debug_folder=None):
        ...
        self.logger.debug(
            "GUI called with: pypogs_system={} gui_update_ms={}".format(
                pypogs_system, gui_update_ms))
        ...
        self.hardware_frame.grid(row=0,
                                 column=0,
                                 columnspan=2,
                                 padx=10,
                                 pady=(10, 0),
        ...
        self.col1_frame.grid(row=1,
                             column=0,
                             padx=(10, 0),
                             pady=0,
                             sticky=tk.E + tk.W + tk.N)
        self.col2_frame = ttk.Frame(self.root)
        self.col2_frame.grid(row=1,
                             column=1,
                             padx=(0, 0),
                             pady=0,
                             sticky=tk.E + tk.W + tk.N)
        self.logger.debug("Loading MountControlFrame")
        self.manual_control_frame = MountControlFrame(self.col1_frame,
                                                      self.sys, self.logger)
        self.manual_control_frame.grid(column=0,
                                       pady=(0, 10),
                                       sticky=tk.E + tk.W + tk.N)
        self.logger.debug("Loading AlignmentFrame")
        self.alignment_frame = AlignmentFrame(self.col1_frame, self.sys,
                                              self.logger)
        self.alignment_frame.grid(column=0,
                                  pady=(0, 10),
                                  sticky=tk.E + tk.W + tk.N)
        ...
        self.target_frame.grid(column=0,
                               pady=(0, 10),
                               sticky=tk.E + tk.W + tk.N)
        self.target_frame.start(gui_update_ms)
        self.logger.debug("Loading ControlPropertiesFrame")
        self.control_prop_frame = ControlPropertiesFrame(
            self.col1_frame, self.sys, self.logger)
        self.control_prop_frame.grid(column=0,
                                     pady=(0, 10),
                                     sticky=tk.E + tk.W + tk.N)
        self.logger.debug("Loading TrackingControlFrame")
        self.tracking_control_frame = TrackingControlFrame(
            self.col1_frame, self.sys, self.logger)
        self.tracking_control_frame.grid(column=0,
                                         pady=(0, 10),
                                         sticky=tk.E + tk.W + tk.N)
        ...
        self.logger.debug(
            "Setting delete function and starting tkinter mainloop")
...
class ControlPropertiesFrame(ttk.Frame):
    ...
    def __init__(self, master, pypogs_system, logger):
        ...
        ttk.Label(self, text="Controller Properties").pack(fill=tk.BOTH,
                                                           expand=True)
        tk.Frame(self, height=1, bg="gray50").pack(fill=tk.BOTH, expand=True)

        ttk.Button(self,
                   text="Feedback",
                   command=self.controller_callback,
                   width=12).pack(fill=tk.BOTH, expand=True)
        ttk.Button(self,
                   text="Coarse Tracker",
                   command=self.coarse_callback,
                   width=12).pack(fill=tk.BOTH, expand=True)
        ttk.Button(self,
                   text="Fine Tracker",
                   command=self.fine_callback,
                   width=12).pack(fill=tk.BOTH, expand=True)
    ...
    def controller_callback(self):
        try:
            if self.controller_popup is None:
                self.controller_popup = self.ControlPopup(
                    self,
                    device=self.sys.control_loop_thread,
    ...
    def coarse_callback(self):
        try:
            ...
            if self.coarse_popup is None:
                self.coarse_popup = self.ControlPopup(
                    self,
                    device=self.sys.coarse_track_thread.spot_tracker,
            ...
            self.logger.debug("Failed to popup coarse control popup",
                              exc_info=True)
            ErrorPopup(self, err, self.logger)
    ...
    def fine_callback(self):
        try:
            ...
            if self.fine_popup is None:
                self.fine_popup = self.ControlPopup(
                    self,
                    device=self.sys.fine_track_thread.spot_tracker,
                    title="Fine Tracker",
                )
            else:
            ...
            self.logger.debug("Failed to popup fine control popup",
                              exc_info=True)
            ErrorPopup(self, err, self.logger)
    ...
    class ControlPopup(tk.Toplevel):
        ...
        def __init__(self, master, device, title="Control"):
            ...
            ttk.Button(self, text="Set",
                       command=self.set_properties).pack(fill=tk.BOTH,
                                                         expand=True)
            ...
            if property_list is not None:
                ...
                for prop in property_list:
                    ...
                    if row >= maxrows:
                        ...
                        column += 2
                    try:
                        (old_name, entry, old_value,
                         label) = self.property_entries[i]
                        entry.delete(0, "end")
                        label["text"] = ""
                    except IndexError:
            ...
            for name, entry, old_value, label in self.property_entries:
                ...
                if not new_value == old_value:
                    ...
                    try:
                        ...
                        self.logger.debug("Set " + str(name) + " to: " +
                                          str(parsed))
                    except Exception as err:
                        ErrorPopup(self, err, self.logger)
                else:
...
class TrackingControlFrame(ttk.Frame):
    ...
    def __init__(self, master, pypogs_system, logger):
        ...
        ttk.Checkbutton(
            auto_frame,
            text="Coarse",
            variable=self.auto_coarse,
            command=self.coarse_callback,
        ).grid(sticky=tk.W)
        ttk.Checkbutton(auto_frame,
                        text="Fine",
                        variable=self.auto_fine,
                        command=self.fine_callback).grid(sticky=tk.W)
        ttk.Button(self,
                   text="Start Tracking",
    ...
    def update(self):
        ...
        if self.sys.coarse_track_thread is not None:
            self.auto_coarse.set(
                self.sys.coarse_track_thread.auto_acquire_track)
        else:
        ...
        if not self._update_stop:
            self.logger.debug(
                "TrackingControlFrame updating self after {} ms".format(
                    self._update_after))
    ...
    def start(self, after=None):
        ...
        self.logger.debug(
            "TrackingControlFrame got start request with after={}".format(
                after))
        if after is not None:
            self._update_after = after
...
class LiveViewFrame(ttk.Frame):
    ...
    def __init__(self, master, pypogs_system, logger):
        ...
        ttk.Label(self.top_frame,
                  text="Live View and Interactive Control").grid(row=0,
                                                                 column=0,
                                                                 padx=(250, 0))
        self.zoom_variable = tk.IntVar()
        self.zoom_variable.set(1)
        ttk.Label(self.top_frame, text="Zoom:").grid(row=0,
                                                     column=1,
                                                     padx=(50, 0))
        ttk.Radiobutton(self.top_frame,
                        text="1x",
                        variable=self.zoom_variable,
                        value=1).grid(row=0, column=2, padx=(5, 0))
        ttk.Radiobutton(self.top_frame,
                        text="2x",
                        variable=self.zoom_variable,
                        value=2).grid(row=0, column=3, padx=(5, 0))
        ttk.Radiobutton(self.top_frame,
                        text="4x",
                        variable=self.zoom_variable,
                        value=4).grid(row=0, column=4, padx=(5, 0))
        ttk.Radiobutton(self.top_frame,
                        text="8x",
                        variable=self.zoom_variable,
                        value=8).grid(row=0, column=5, padx=(5, 0))
        ...
        ttk.Label(self.bottom_frame1, text="Camera (Tracker):").grid(row=0,
                                                                     column=0)
        self.camera_variable = tk.IntVar()
        self.camera_variable.set(NONE)
        ttk.Radiobutton(self.bottom_frame1,
                        text="None",
                        variable=self.camera_variable,
                        value=NONE).grid(row=0, column=1, padx=(5, 0))
        ttk.Radiobutton(
            self.bottom_frame1,
            text="Star (OL)",
            variable=self.camera_variable,
        ...
        self.logger.debug(
            "Creating entry and checkbox for image max value control")
        ttk.Label(self.bottom_frame1, text="Max Value:").grid(row=0,
                                                              column=6,
                                                              padx=(30, 0))
        ...
        ttk.Checkbutton(self.bottom_frame1,
                        variable=self.auto_max_variable,
                        text="Auto").grid(row=0, column=8, padx=(5, 0))
        ...
        ttk.Checkbutton(self.bottom_frame1,
                        text="Annotate",
                        variable=self.annotate_variable).grid(row=0,
                                                              column=9,
                                                              padx=(30, 0))

        self.set_search_variable = tk.BooleanVar()
        self.set_search_variable.set(False)
        ttk.Checkbutton(self.bottom_frame2,
                        text="Manual Acquire",
                        variable=self.set_search_variable).grid(row=0,
                                                                column=1)
        ttk.Button(self.bottom_frame2,
                   text="Clear Track",
                   command=self.clear_tracker_callback).grid(row=0,
                                                             column=2,
                                                             padx=(5, 0))
        ...
        ttk.Checkbutton(self.bottom_frame2,
                        text="Add Offset",
                        variable=self.add_offset_variable).grid(row=0,
                                                                column=3,
                                                                padx=(50, 0))
        ttk.Button(self.bottom_frame2,
                   text="Clear Offset",
                   command=self.clear_offset_callback).grid(row=0,
                                                            column=4,
                                                            padx=(5, 0))
        ...
        ttk.Checkbutton(
            self.bottom_frame2,
            text="Intercam Alignment",
            variable=self.set_goal_variable,
    ...
    def clear_tracker_callback(self):
        ...
        if cam == STAR_OL:
            ...
            try:  # Only set offset to zero, there is nothing else to do
                ...
                self.logger.debug("Could not clear coarse tracker",
                                  exc_info=True)
                ErrorPopup(self, err, self.logger)
        elif cam == COARSE_CCL:
            ...
            try:
                ...
                self.logger.debug("Could not clear fine tracker",
                                  exc_info=True)
                ErrorPopup(self, err, self.logger)
        else:
    ...
    def click_canvas_callback(self, event):
        ...
        if event.x > self.image_size[0] or event.y > self.image_size[1]:
            ...
            y_image = (self.image_size[1] / 2 - event.y
                       )  # Camera coordinates are opposite
            self.logger.debug("Image coordinates x:" + str(x_image) + " y:" +
                              str(y_image))
        if self.set_goal_variable.get() and not None in (x_image, y_image):
            cam = self.camera_variable.get()
            if cam == STAR_OL:
                self.logger.debug(
                    "Setting goal for OL tracker from Star camera")
                try:
                    plate_scale = self.sys.star_camera.plate_scale
                    goal = [
                        x_image / self.image_scale * plate_scale,
                        y_image / self.image_scale * plate_scale,
                    ]
                ...
                self.logger.debug(
                    "Setting goal for CCL tracker from Coarse camera")
                try:
                ...
                self.logger.debug(
                    "Setting goal for FCL tracker from Fine camera")
                try:
                ...
                self.logger.debug(
                    "No camera selected in canvas click callback")
            self.set_goal_variable.set(False)
        elif self.add_offset_variable.get() and not None in (x_image, y_image):
            cam = self.camera_variable.get()
            if cam == COARSE_CCL:
                self.logger.debug(
                    "Setting offset for OL tracker from Coarse camera")
                try:
                    plate_scale = self.sys.coarse_camera.plate_scale
                    ...
                    offset = np.array(click) - np.array(
                        self.sys.coarse_track_thread.goal_x_y)
                    rotmx = np.array([
                        [
                    ...
                    offset = rotmx @ offset
                    self.logger.info("Subtracting from OL offset: " +
                                     str(offset))
                    old_offset = self.sys.control_loop_thread.OL_goal_offset_x_y
                    self.sys.control_loop_thread.OL_goal_offset_x_y = (
                        np.array(old_offset) - offset)
                except Exception as err:
                    self.logger.debug("Could not set OL offset", exc_info=True)
                    ErrorPopup(self, err, self.logger)
            elif cam == FINE_FCL:
                self.logger.debug(
                    "Setting offset for CCL tracker from Fine camera")
                try:
                    plate_scale = self.sys.fine_camera.plate_scale
                    ...
                    offset = np.array(click) - np.array(
                        self.sys.fine_track_thread.goal_x_y)
                    rotmx1 = np.array([
                        [
                            np.cos(np.deg2rad(rotation_fine)),
                            np.sin(np.deg2rad(rotation_fine)),
                        ],
                        [
                            -np.sin(np.deg2rad(rotation_fine)),
                            np.cos(np.deg2rad(rotation_fine)),
                    ...
                    rotmx2 = np.array([
                        [
                            np.cos(np.deg2rad(rotation_coarse)),
                            -np.sin(np.deg2rad(rotation_coarse)),
                        ],
                        [
                            np.sin(np.deg2rad(rotation_coarse)),
                            np.cos(np.deg2rad(rotation_coarse)),
                    ...
                    self.logger.info("Subtracting from CCL offset: " +
                                     str(offset))
                    old_offset = self.sys.coarse_track_thread.goal_offset_x_y
                    self.sys.coarse_track_thread.goal_offset_x_y = (
                        np.array(old_offset) - offset)
                except Exception as err:
                    self.logger.debug("Could not set CCL offset",
                                      exc_info=True)
                    ErrorPopup(self, err, self.logger)
            else:
                self.logger.debug(
            ...
            if cam == COARSE_CCL:
                ...
                self.logger.info("Setting coarse search position to: " +
                                 str(search_pos))
                ...
                self.logger.info("Setting fine search position to: " +
                                 str(search_pos))
    ...
    def update(self):
        ...
        cam = self.camera_variable.get()
        self.logger.debug(
            "Selected camera is {} (1 star, 2 coarse, 3 fine, 0 none)".format(
                cam))
        # Check if we have it, otherwise switch to none
        if cam == STAR_OL and (self.sys.star_camera is None
                               or not self.sys.star_camera.is_init):
            cam = NONE
        if cam == COARSE_CCL and (self.sys.coarse_camera is None
                                  or not self.sys.coarse_camera.is_init):
            cam = NONE
        if cam == FINE_FCL and (self.sys.fine_camera is None
                                or not self.sys.fine_camera.is_init):
            cam = NONE
        self.logger.debug(
            "After validity checks setting camera to {}".format(cam))
        ...
        if cam == STAR_OL:
            ...
            try:
                if not self.sys.star_camera.is_running:
                    self.sys.star_camera.start()
                ...
                try:
                    offset_pos = np.array(goal_pos) + np.array(
                        self.sys.control_loop_thread.OL_goal_offset_x_y)
                except:
            ...
            try:
                if not self.sys.coarse_camera.is_running:
                    self.sys.coarse_camera.start()
            ...
            try:
                if not self.sys.fine_camera.is_running:
                    self.sys.fine_camera.start()
                img = self.sys.fine_camera.get_latest_image()
                goal_pos = self.sys.fine_track_thread.goal_x_y
                try:
                    offset_pos = np.array(goal_pos) + np.array(
                        self.sys.fine_track_thread.goal_offset_x_y)
                except:
                    self.logger.debug("Could not get offset", exc_info=True)
                plate_scale = self.sys.fine_camera.plate_scale
        ...
        if img is not None:
            if self.auto_max_variable.get():  # Auto set max scaling
                ...
                self.logger.debug(
                    "Using auto max scaling with maxval {}".format(maxval))
            else:
                try:
                    ...
                    self.logger.debug(
                        "Failed to convert max entry value {} to int".format(
                            self.max_entry.get()))
                    maxval = 255
            # Scale and convert to 8-bit
            img_scaled = ((img / max(1, int(0.8 * maxval / 255))).clip(
                0, 255).astype("uint8"))
            pil_img = Image.fromarray(img_scaled)
            # Resize to fit inside canvas
            (in_width, in_height) = pil_img.size
            self.image_scale = min(self.canvas_size[0] / in_width,
                                   self.canvas_size[1] / in_height)
            self.image_size = (
                round(self.image_scale * in_width),
                round(self.image_scale * in_height),
            )
            ...
            if self.annotate_variable.get() and not None in (
                    goal_pos[0],
                    goal_pos[1],
                    plate_scale,
            ):
                self.logger.debug("Annotating Goal: " + str(goal_pos) +
                                  " scale: " + str(plate_scale))
                gap = 5
                length = 30
                width = 4
                xhair_x = (goal_pos[0] / plate_scale * self.image_scale +
                           self.image_size[0] / 2)
                xhair_y = (-goal_pos[1] / plate_scale * self.image_scale +
                           self.image_size[1] / 2)
                self.logger.debug("Crosshair goes at: " +
                                  str((xhair_x, xhair_y)))
                xhair = np.array([
                    [0, -gap],
                    [0, -gap - length - 2],
                ...
                xhair = (rotmx @ xhair) + np.array([xhair_x, xhair_y]).reshape(
                    (2, 1))

                if None in self.goal_handles:
                    self.goal_handles[0] = self.canvas.create_line(
                        *xhair[:, 0],
                        *xhair[:, 1],
                        fill="blue",
                        width=width,
                        ...
                        arrow=tk.LAST,
                        arrowshape=(10, 14, 6))
                    self.goal_handles[1] = self.canvas.create_line(*xhair[:,
                                                                          2],
                                                                   *xhair[:,
                                                                          3],
                                                                   fill="blue",
                                                                   width=width,
                                                                   tag="goal")
                    self.goal_handles[2] = self.canvas.create_line(
                        *xhair[:, 4],
                        *xhair[:, 5],
                        fill="blue",
                        width=width,
                        ...
                        arrow=tk.LAST,
                        arrowshape=(10, 14, 6))
                    self.goal_handles[3] = self.canvas.create_line(*xhair[:,
                                                                          6],
                                                                   *xhair[:,
                                                                          7],
                                                                   fill="blue",
                    ...
                    self.canvas.coords(self.goal_handles[0], *xhair[:, 0],
                                       *xhair[:, 1])
                    self.canvas.coords(self.goal_handles[1], *xhair[:, 2],
                                       *xhair[:, 3])
                    self.canvas.coords(self.goal_handles[2], *xhair[:, 4],
                                       *xhair[:, 5])
                    self.canvas.coords(self.goal_handles[3], *xhair[:, 6],
                                       *xhair[:, 7])
            ...
            if self.annotate_variable.get() and not None in (
                ...
                self.logger.debug("Annotating Offset: " + str(offset_pos) +
                                  " scale: " + str(plate_scale))
                gap = 5
                length = 10
                width = 4
                xhair_x = (offset_pos[0] / plate_scale * self.image_scale +
                           self.image_size[0] / 2)
                xhair_y = (-offset_pos[1] / plate_scale * self.image_scale +
                           self.image_size[1] / 2)
                self.logger.debug("Crosshair goes at: " +
                                  str((xhair_x, xhair_y)))
                xhair = np.array([
                ...
                if None in self.offset_handles:
                    self.offset_handles[0] = self.canvas.create_line(
                        *xhair[:, 0],
                        *xhair[:, 1],
                        fill="royal blue",
                        width=width,
                        tag="offset")
                    self.offset_handles[1] = self.canvas.create_line(
                        *xhair[:, 2],
                        *xhair[:, 3],
                        fill="royal blue",
                        width=width,
                        tag="offset")
                    self.offset_handles[2] = self.canvas.create_line(
                        *xhair[:, 4],
                        *xhair[:, 5],
                        fill="royal blue",
                        width=width,
                        tag="offset")
                    self.offset_handles[3] = self.canvas.create_line(
                        *xhair[:, 6],
                        *xhair[:, 7],
                        fill="royal blue",
                        width=width,
                    ...
                    self.canvas.coords(self.offset_handles[0], *xhair[:, 0],
                                       *xhair[:, 1])
                    self.canvas.coords(self.offset_handles[1], *xhair[:, 2],
                                       *xhair[:, 3])
                    self.canvas.coords(self.offset_handles[2], *xhair[:, 4],
                                       *xhair[:, 5])
                    self.canvas.coords(self.offset_handles[3], *xhair[:, 6],
                                       *xhair[:, 7])
            ...
            if self.annotate_variable.get() and not None in (
                ...
                self.logger.debug("Annotating Mean: " + str(mean_pos) +
                                  " scale: " + str(track_sd))
                canvas_x = (mean_pos[0] / plate_scale * self.image_scale +
                            self.image_size[0] / 2 - 0.5)
                canvas_y = (-mean_pos[1] / plate_scale * self.image_scale +
                            self.image_size[1] / 2 + 0.5)
                ...
                self.logger.debug("Circle goes at: " +
                                  str((canvas_x, canvas_y)) + " radius: " +
                                  str(canvas_sd))
                colour = "green"
                if self.track_circle_handle is None:
                    self.track_circle_handle = self.canvas.create_oval(
                        *coords, outline=colour, width=2, tag="mean")
                else:
            ...
            if self.annotate_variable.get() and not None in (
                ...
                self.logger.debug("Annotating Search: " + str(search_pos) +
                                  " radius: " + str(search_rad))
                canvas_x = (search_pos[0] / plate_scale * self.image_scale +
                            self.image_size[0] / 2 - 0.5)
                canvas_y = (-search_pos[1] / plate_scale * self.image_scale +
                            self.image_size[1] / 2 + 0.5)
                ...
                if self.search_circle_handle is None:
                    self.search_circle_handle = self.canvas.create_oval(
                        *coords, outline=colour, width=2, tag="search")
                else:
            ...
            if self.annotate_variable.get() and not None in (
                ...
                self.logger.debug("Annotating Search: " + str(track_pos) +
                                  " radius: " + str(search_rad))
                canvas_x = (track_pos[0] / plate_scale * self.image_scale +
                            self.image_size[0] / 2 - 0.5)
                canvas_y = (-track_pos[1] / plate_scale * self.image_scale +
                            self.image_size[1] / 2 + 0.5)
                length = 6
                width = 4
                colour = "green"
                coords_h = (canvas_x - length, canvas_y, canvas_x + length,
                            canvas_y)
                coords_v = (canvas_x, canvas_y - length, canvas_x,
                            canvas_y + length)
                self.logger.debug("Cross goes at: " +
                                  str((canvas_x, canvas_y)))
                if None in self.track_cross_handles:
                    self.track_cross_handles[0] = self.canvas.create_line(
                        coords_h, fill=colour, width=width, tag="track")
                    self.track_cross_handles[1] = self.canvas.create_line(
                        coords_v, fill=colour, width=width, tag="track")
                else:
            ...
            self.tk_image = ImageTk.PhotoImage(
                image=Image.new(mode="L", size=self.canvas_size))
            self.image_size = self.canvas_size
            if self.canvas_image is None:
                self.logger.debug("Creating image on canvas")
                self.canvas_image = self.canvas.create_image(
        ...
        if not self._update_stop:
            self.logger.debug("Calling update on self after {} ms".format(
                self._update_after))
            self.after(self._update_after, self.update)
    ...
    def start(self, after=None):
        """Give number of milliseconds to wait between updates."""
        self.logger.debug(
            "LiveViewFrame got start request with after={}".format(after))
        if after is not None:
            self._update_after = after
        self._update_stop = False
        self.update()
...
class HardwareFrame(ttk.Frame):
    ...
    def __init__(self, master, pypogs_system, logger):
        ...
        ttk.Label(self, text="Hardware Setup and Properties").grid(
            row=0, column=0, columnspan=6, sticky=tk.W + tk.E)
        tk.Frame(self, height=1, bg="gray50").grid(row=1,
                                                   column=0,
                                                   columnspan=6,
                                                   sticky=tk.W + tk.E)
        ...
        self.star_button = ttk.Button(
            ...
            text="Star Cam\n(Not set)",
            style="star.TButton",
            command=self.star_callback,
            width=12,
        )
        self.star_button.grid(row=2, column=1)
        self.coarse_button = ttk.Button(
            self,
            text="Coarse Cam\n(Not set)",
            style="coarse.TButton",
            command=self.coarse_callback,
            width=12,
        )
        self.coarse_button.grid(row=2, column=2)
        self.fine_button = ttk.Button(
            self,
            text="Fine Cam\n(Not set)",
            style="fine.TButton",
            command=self.fine_callback,
        ...
        self.receiver_button = ttk.Button(
            ...
            text="Receiver\n(Not set)",
            style="rec.TButton",
            command=self.receiver_callback,
            width=12,
        )
        ...
        ttk.Button(self.init_frame,
                   text="Initialize All",
                   command=self.init_callback,
                   width=12).pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        ttk.Button(
            self.init_frame,
            text="Deinitialize All",
            command=self.deinit_callback,
            width=12,
    ...
    def mount_callback(self):
        ...
        try:
            if self.mount_popup is None:
                self.mount_popup = self.HardwarePopup(
                    self,
                    self.sys.mount,
                    self.sys.add_mount,
                    self.sys.clear_mount,
                    title="Mount",
                    default_name="UnnamedMount",
                )
            else:
    ...
    def star_callback(self):
        ...
        try:
            if self.star_popup is None:
                self.star_popup = self.HardwarePopup(
                    self,
                    self.sys.star_camera,
                    self.sys.add_star_camera,
                    self.sys.clear_star_camera,
                    title="Star camera",
                    default_name="StarCamera",
                    link_device=self.sys.coarse_camera,
    ...
    def coarse_callback(self):
        ...
        try:
            if self.coarse_popup is None:
                self.coarse_popup = self.HardwarePopup(
                    self,
                    self.sys.coarse_camera,
                    self.sys.add_coarse_camera,
                    self.sys.clear_coarse_camera,
                    title="Coarse camera",
                    default_name="CoarseCamera",
                    link_device=self.sys.star_camera,
    ...
    def fine_callback(self):
        ...
        try:
            if self.fine_popup is None:
                self.fine_popup = self.HardwarePopup(
                    self,
                    self.sys.fine_camera,
                    self.sys.add_fine_camera,
                    self.sys.clear_fine_camera,
                    title="Fine camera",
    ...
    def receiver_callback(self):
        ...
        try:
            if self.receiver_popup is None:
                self.receiver_popup = self.HardwarePopup(
                    self,
                    self.sys.receiver,
                    self.sys.add_receiver,
                    self.sys.clear_receiver,
                    title="Receiver",
    ...
    class HardwarePopup(tk.Toplevel):
        ...
        def __init__(
            ...
            r = 0
            if link_device != 0:
                ttk.Checkbutton(setup_frame,
                                text="Link Star and Coarse",
                                variable=self.linked_bool).grid(row=r,
                                                                column=0)
                r += 1

            ttk.Label(setup_frame, text="Model:").grid(row=r, column=0)
            r += 1
            self.model_entry = ttk.Entry(setup_frame, width=20)
            self.model_entry.grid(row=r, column=0)
            r += 1
            ttk.Label(setup_frame, text="Identity:").grid(row=r, column=0)
            r += 1
            self.identity_entry = ttk.Entry(setup_frame, width=20)
            self.identity_entry.grid(row=r, column=0)
            r += 1
            ttk.Label(setup_frame, text="Name:").grid(row=r, column=0)
            r += 1
            self.name_entry = ttk.Entry(setup_frame, width=20)
            self.name_entry.grid(row=r, column=0)
            r += 1
            ttk.Button(setup_frame, text="Add",
                       command=self.connect_callback).grid(row=r,
                                                           column=0,
                                                           sticky=tk.W + tk.E)
            r += 1
            ttk.Button(setup_frame, text="Remove",
                       command=self.clear_callback).grid(row=r,
                                                         column=0,
                                                         sticky=tk.W + tk.E)
            r += 1
            ...
            self.properties_frame = ttk.Frame(self)
            self.property_entries = []
            self.properties_label = ttk.Label(self.properties_frame,
                                              text="Device properties:")
            self.set_prop_button = ttk.Button(self.properties_frame,
                                              text="Set",
        ...
        def update(self):
            ...
            self.linked_bool.set(self.device is not None
                                 and self.device is self.link_device)
            self.master.update()
        ...
        def connect_callback(self):
            ...
            try:
                self.device = self.add_func(model=model,
                                            identity=identity,
                                            name=name)
            except Exception as err:
                ErrorPopup(self, err, self.logger)
        ...
        def update_properties(self):
            ...
            if property_list is not None:
                ...
                for prop in property_list:
                    ...
                    row += 1
                    i += 1
                self.properties_label.grid(row=0,
                                           column=1,
                                           columnspan=column + 1)
                self.set_prop_button.grid(row=maxrows + 1,
                                          column=1,
                                          columnspan=column + 1,
                                          sticky=tk.W + tk.E)
                self.properties_frame.grid(row=0,
                                           column=1,
                                           padx=(20, 0),
                                           sticky=tk.S)
            else:
                self.properties_frame.grid_forget(
                )  # Hide entirely from the popup
            # Hide any widgets no longer in use
            while i < len(self.property_entries):
                (old_name, entry, old_value, label) = self.property_entries[i]
                entry.grid_forget()
            ...
            for name, entry, old_value, label in self.property_entries:
                ...
                if not new_value == old_value:
                    try:
                        ...
                        self.logger.debug("Set " + str(name) + " to: " +
                                          str(parse_expression(new_value)))
                    except Exception as err:
                        ErrorPopup(self, err, self.logger)
                else:
                    self.logger.debug("Did not change: " + str(name))
...
class TargetFrame(ttk.Frame):
    ...
    def __init__(self, master, pypogs_system, logger):
        ...
        ttk.Button(self,
                   text="Set Manual",
                   command=self.manual_button_callback,
                   width=15).grid(row=3, column=0)
        ttk.Button(self,
                   text="Set from File",
    ...
    def update(self):
        ...
        start_string = (("Start: " + t_start.strftime("%H:%M:%S"))
                        if t_start is not None else "No start time")
        t_end = self.sys.target.end_time
        end_string = (("End: " + t_end.strftime("%H:%M:%S"))
                      if t_end is not None else "No end time")
        try:
            itrf_xyz = self.sys.get_itrf_direction_of_target()
            enu_altaz = self.sys.alignment.get_enu_altaz_from_itrf_xyz(
                itrf_xyz)
            altaz_string = ("Alt:" + str(round(enu_altaz[0], 1)) + DEG +
                            " Az:" + str(round(enu_altaz[1], 1)) + DEG)
    ...
    class ManualPopup(tk.Toplevel):
        ...
        def __init__(self, master):
            ...
            tle_frame.grid(row=0,
                           column=0,
                           columnspan=2,
                           padx=(0, 10),
                           pady=10)
            ttk.Label(tle_frame, text="Set target from TLE:").grid(row=0,
                                                                   column=0)
            self.tle_line1_entry = ttk.Entry(tle_frame,
                                             width=69,
                                             font="TkFixedFont")
            self.tle_line1_entry.grid(row=1, column=0)
            self.tle_line2_entry = ttk.Entry(tle_frame,
                                             width=69,
                                             font="TkFixedFont")
            self.tle_line2_entry.grid(row=2, column=0)
            ttk.Button(tle_frame, text="Set",
                       command=self.tle_callback).grid(row=3,
                                                       column=0,
                                                       sticky=tk.W + tk.E)

            radec_frame = ttk.Frame(self)
            radec_frame.grid(row=1, column=0, padx=(10, 0), pady=10)
            ttk.Label(radec_frame,
                      text="Set target from RA/Dec:").grid(row=0,
                                                           column=0,
                                                           columnspan=2)
            ttk.Label(radec_frame, text="RA: (deg)").grid(row=1,
                                                          column=0,
                                                          sticky=tk.E)
            self.ra_entry = ttk.Entry(radec_frame,
                                      width=25,
                                      font="TkFixedFont")
            self.ra_entry.grid(row=1, column=1)
            ttk.Label(radec_frame, text="Dec: (deg)").grid(row=2,
                                                           column=0,
                                                           sticky=tk.E)
            self.dec_entry = ttk.Entry(radec_frame,
                                       width=25,
                                       font="TkFixedFont")
            self.dec_entry.grid(row=2, column=1)
            ttk.Button(radec_frame, text="Set",
                       command=self.radec_callback).grid(row=3,
                                                         column=1,
                                                         sticky=tk.W + tk.E)
            ...
            ttk.Label(time_frame,
                      text="Set tracking time (optional):").grid(row=0,
                                                                 column=0,
                                                                 columnspan=3)
            ttk.Label(time_frame, text="Start: (UTC)").grid(row=1,
                                                            column=0,
                                                            sticky=tk.E)
            self.start_entry = ttk.Entry(time_frame,
                                         width=25,
                                         font="TkFixedFont")
            self.start_entry.grid(row=1, column=1, columnspan=2)
            ttk.Label(time_frame, text="End: (UTC)").grid(row=2,
                                                          column=0,
                                                          sticky=tk.E)
            self.end_entry = ttk.Entry(time_frame,
                                       width=25,
                                       font="TkFixedFont")
            self.end_entry.grid(row=2, column=1, columnspan=2)
            ttk.Button(time_frame,
                       text="Set",
                       width=10,
                       command=self.set_time_callback).grid(row=3,
                                                            column=1,
                                                            sticky=tk.W + tk.E)
            ttk.Button(time_frame,
                       text="Clear",
                       width=10,
                       command=self.clear_time_callback).grid(row=3,
                                                              column=2,
                                                              sticky=tk.W +
                                                              tk.E)
        ...
        def update(self):
            ...
            self.start_entry.insert(
                0,
                str(t_start) if t_start is not None else "")
...
class AlignmentFrame(ttk.Frame):
    ...
    def __init__(self, master, pypogs_system, logger):
        ...
        self._update_stop = True
        # Create widgets and layout
        ttk.Label(self, text="Location and Alignment").grid(row=0,
                                                            column=0,
                                                            columnspan=2)
        tk.Frame(self, height=1, bg="gray50").grid(row=1,
                                                   column=0,
        ...
        ttk.Button(self,
                   text="Set Location",
                   command=self.location_button_callback,
                   width=15).grid(row=3, column=0)
        ttk.Button(self,
                   text="Set Alignment",
                   command=self.alignment_button_callback,
                   width=15).grid(row=3, column=1)
    ...
    def update(self):
        ...
        try:
            (lat, lon,
             height) = self.sys.alignment.get_location_lat_lon_height()
            loc_string = ("N:" + str(round(lat, 3)) + DEG + " E:" +
                          str(round(lon, 3)) + DEG)
        except AssertionError:
    ...
    class LocationPopup(tk.Toplevel):
        ...
        def __init__(self, master):
            ...
            ttk.Button(self, text="Set", width=20,
                       command=self.set_callback).grid(row=4,
                                                       column=0,
                                                       columnspan=2)
            self.protocol("WM_DELETE_WINDOW", self.withdraw)
        ...
        def set_callback(self):
            try:
                ...
                self.master.sys.alignment.set_location_lat_lon(
                    lat, lon, height)
            #                self.master.update()
            #                self.withdraw()
            except Exception as err:
    ...
    class AlignmentPopup(tk.Toplevel):
        ...
        def __init__(self, master):
            ...
            self.resizable(False, False)
            #            self.grab_set() #Grab control
            ttk.Label(self, text="Set Alignment").grid(row=0,
                                                       column=0,
                                                       columnspan=3)
            ttk.Button(self,
                       text="Do auto-align",
                       width=18,
                       command=self.auto_callback).grid(row=1, column=0)
            ttk.Button(self,
                       text="EastNorthUp",
                       width=18,
                       command=self.enu_callback).grid(row=1, column=1)
            ttk.Button(self,
                       text="Load from file",
                       width=18,
                       command=self.load_callback).grid(row=1, column=2)
            self.protocol("WM_DELETE_WINDOW", self.withdraw)
...
class MountControlFrame(ttk.Frame):
    ...
    def __init__(self, master, pypogs_system, logger):
        ...
        self.sys = pypogs_system
        # Create widgets and layout
        ttk.Label(self, text="Manual Control").grid(row=0,
                                                    column=0,
                                                    columnspan=2)
        tk.Frame(self, height=1, bg="gray50").grid(row=1,
                                                   column=0,
        ...
        ttk.Radiobutton(rb_frame,
                        text="COM",
                        variable=self.coord_variable,
                        value=COMMAND).grid(sticky=tk.W)
        ttk.Radiobutton(rb_frame,
                        text="MNT",
                        variable=self.coord_variable,
                        value=MOUNT).grid(sticky=tk.W)
        ttk.Radiobutton(rb_frame,
                        text="ENU",
                        variable=self.coord_variable,
                        value=ENU).grid(sticky=tk.W)
    ...
    def stop_button_callback(self):
        ...
        assert (self.sys.mount is not None
                and self.sys.mount.is_init), "No mount or not initialised"
        self.sys.mount.stop()
    ...
    def send_button_callback(self):
        ...
        try:
            if input_ok:
                # Check mount ready
                assert (self.sys.mount is not None and
                        self.sys.mount.is_init), "No mount or not initialised"
                ...
                if mode == COMMAND:
                    ...
                    com_altaz = self.sys.alignment.get_com_altaz_from_mnt_altaz(
                        (alt, azi))
                    ...
                    com_altaz = self.sys.alignment.get_com_altaz_from_enu_altaz(
                        (alt, azi))
                    self.logger.debug("Command: " + str(com_altaz))
                    self.sys.mount.move_to_alt_az(*com_altaz, block=False)
                else:
